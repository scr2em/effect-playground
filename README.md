...
5. **Fix it**: faded worked examples. Early challenges are missing one call, later ones are missing more. Some fail only as a type error, which is the point of Effect. Hints are progressive. The explanation after success says *why* the fix works.
6. **Build it**: small real programs from a spec with exact expected output. This is the transfer step.
7. **Recall**: retrieval questions to answer in your head. Come back to them the next day.

Progress and drafts are stored in your browser's localStorage. "Reset progress" in the sidebar clears them.
